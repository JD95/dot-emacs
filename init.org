...
(setq org-agenda-custom-commands
  '(("!" "Expired Deadlines" tags-todo
    ...
    ("d" "Scheduled or Deadline" agenda
     "+DEADLINE+SCHEDULED"
     nil nil)
    ("u" "Upcoming" ((org-ql-block '(deadline :to 62))))))
#+end_src
...
Truncate lines after the habits graph

#+begin_src emacs-lisp
(defun truncate-org-agenda-entries ()
  (save-excursion
    (goto-char (point-min))
    (while (not (eobp))
      (when (seq-contains (thing-at-point 'line t) ?!)
        (move-to-column 62)
        (zap-up-to-char 1 ?\n))
      (forward-line 1))))

(add-hook 'org-agenda-finalize-hook #'truncate-org-agenda-entries)
#+end_src
...
(setq org-todo-keywords
  '((sequence "TODO(t)" "|" "DONE(d)")
    ...
    (sequence "UNKNOWN(u)" "RESEARCHING(r)" "|" "ANSWERED(v)")
...
(setq org-super-agenda-groups '(
  (:name "Time Sensitive" :tag "TimeSensitive")))
